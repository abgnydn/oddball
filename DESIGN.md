...
does not exist in a standalone build, and nothing else. Three of the 8 rest on harness
...
| Holding Enter opens pause from anywhere in gameplay, with a visible and audible indication while holding | met. The hold works everywhere including flight, and from 1200 ms a ring appears over the play area and fills to the 3000 ms threshold while a blip a step higher each second makes the same progress audible. §4 names Race Tracks as the build to copy; this matches its shape (ring at 2000 of 5000 ms there, the same fraction here) and its beep figures. The pointer hold and the Space backward-scan hold drive the same indicator |
...
| Settings reachable from **both** the main menu and the pause menu | met, including during the flight animation. The row used to be dropped in flight because there was no route back out of it — the shot list is gone and the ball is mid-animation — so Back from Settings now restores the flight's own screen state and reopens the paused menu. `menu-check` asserts the row is there, that it reaches Settings, and that Back lands on the paused menu rather than stranding the player |
...
| Reset Progress is two-step | met. Last row in Settings; arms on the first pick, acts on the second, and the confirm is guarded against a bounce. Clears settings, the round, saved holes, the editor draft and best scores |
| Exit Game sends `postMessage({ action: 'focusBackButton' })` | met. Exit posts `{ action: 'focusBackButton' }` to the parent when this build is running in the hub's iframe, guarded on `window.parent !== window` the way their own games guard it, and falls back to this build's title when standalone |
...
| Anything mouse-only or off-site sits behind a spoken confirm dialog, "with Cancel first in the scan order and the scan trapped in the dialog" | met, though the trigger does not arise — nothing here is mouse-only, nothing leaves the page and the editor is fully scannable. The shape is followed anyway by the three two-step confirms this build does have: armed, each becomes a two-row dialog with the cancel answer first and focus starting on it, and nothing else on the list, so over-scanning cannot reach the destructive answer or wrap onto it from the far side. `menu-check` asserts the order, the trapping and the opening focus |
...
- **`layout-check` measures five screens; the game has more.** The course
  ...
  the two end-of-round summaries and the in-round pause menu — five rows, where
  the `overlay` cells open the two-row title one — are visited by no cell. The
  course picker, the two-player picker, the editor, Help and My Holes ARE
  measured now, reached by click paths. The summaries need a real hole-out to
  reach and are not automated; a rendering review swept them by hand. That is a known region rather than an unruled-out one.
